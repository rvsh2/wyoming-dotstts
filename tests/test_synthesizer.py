...
class _FakeRuntime:
    ...
    def generate_stream(self, **kwargs):
        ...
        yield _FakeTensor([[0.1, -0.1]])
        yield _FakeTensor([[0.2]])
...
class SynthesizerTests(unittest.TestCase):
    ...
    def test_synthesize_passes_runtime_arguments(self):
        ...
        self.assertEqual(result.audio, [0.0, 0.25, -0.25])
    ...
    def test_synthesize_stream_yields_float_lists(self):
        temp = self._speaker_dir()
        self.addCleanup(temp.cleanup)
        runtime = _FakeRuntime()
        ...
        chunks = list(synth.synthesize_stream("Streaming.", voice_name="mira"))

        self.assertEqual(chunks, [([0.1, -0.1], 48000), ([0.2], 48000)])
